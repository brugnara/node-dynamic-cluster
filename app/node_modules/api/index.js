...
const debug = Debug('node-dynamic-cluster:api');
debug.verbose = Debug('verbose:node-dynamic-cluster:api');
...
module.exports = function Api(options) {
  ...
  const app = http.createServer((req, res) => {
    ...
    const matchCheck = /^\/check$/i.exec(urlParts.pathname);
    debug.verbose('matchCheck result: %j', matchCheck);

    let _count;

    if (matchCheck) {
      const ok = size === Object.keys(cluster.workers).length;
      return res.end(JSON.stringify({
        status: ok ? 'ok' : 'ko'
      }));
    }

    if (matchInfo || matchResize) {
    ...
    if (matchResize) {
      ...
      if (maxSize && _count > maxSize) {
        return res.end(JSON.stringify({
          status: 'ko',
          error: `wanted size (${_count}) exceeds the maxSize (${maxSize})`
        }));
      }
  ...
  return {
    ...
    listen: function (cb) {
      ...
      cluster.on('exit', (worker, code, signal) => {
        debug(`worker ${worker.id} (pid: ${worker.process.pid}) died with signal: ${signal} and code: ${code}.`);
